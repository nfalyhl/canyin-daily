...
(function () {
  ...
  var AUTH = {
    ...
    base: '',             // 鉴权服务地址（remote）
    cfg: {},              // 页面里内联的鉴权配置原文
    protected: false,     // true = 正文不内联，登录后从服务端取
    user: '',
    deviceId: '',
    devices: [],
    maxDevices: 3,
  ...
  /* --------------------------------------------------------------- 网络 --- */
  function once(url, payload, timeout) {
    var ctl = (typeof AbortController !== 'undefined') ? new AbortController() : null;
    var t = ctl ? setTimeout(function () { ctl.abort(); }, timeout || 20000) : null;
    return fetch(url, {
  ...
  /** 带重试：*.deno.net 在国内时好时坏（实测直连 6 次里挂 4 次），
      只在连接层失败时重试，服务器回的错误码直接抛出去。 */
  function req(url, payload, timeout) {
    var tries = 0;
    function go() {
      tries += 1;
      return once(url, payload, timeout).catch(function (e) {
        if (e && e.status === 0 && tries < 3) {
          return new Promise(function (res) { setTimeout(res, 500 * tries); }).then(go);
        }
        throw e;
      });
    }
  ...
  // 给 app.js 用的两个钩子
  AUTH.api = api;
  AUTH.token = function () { return store(TOK_KEY); };
  ...
  function startRemote(cfg) {
    ...
    AUTH.enabled = true;
    AUTH.cfg = cfg;
    AUTH.protected = !!cfg.protectContent;
